...
model1.load_state_dict(torch.load('trained_model_diff.pth', weights_only=True))
if test_second:
...
while True:
    ...
    if coordinateStore1.new:
        ...
        # inp = torch.tensor([coordinateStore1.point[0]-x, coordinateStore1.point[1]-y]).float()
        inp = torch.tensor([coordinateStore1.point[0]-x, coordinateStore1.point[1]-y]).float()
        print("click_point",coordinateStore1.point)
        print("inp",inp)
        print("x,y",x,y)
        ...
        print("t",t)
        print("t01",t0,t1)
        api.setPos(-abs(t0+t[0]), t1+t[1], module)
        print("set_pos",-abs(t0+t[0]), t1+t[1])
        print("\n"*3)
        # if not test_second:
        #     continue
