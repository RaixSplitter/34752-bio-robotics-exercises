...
# data = np.loadtxt("data_center.csv", delimiter=",")
# end_pos = data[:,2:].T
# x = torch.from_numpy(end_pos.T).float()
# data_mean = x.mean(axis=0)
# data_mean = torch.tensor([-1.4900, -0.8800]).float()
data_mean = torch.tensor([-46.9600,  -3.2021,  -1.8200,   1.3300]).float()
...
model1 = torch_model.MLPNet(4, 24, 2)
# model1.load_state_dict(torch.load('280_trained_model.pth'))
model1.load_state_dict(torch.load('trained_model_diffs.pth'))
if test_second:
...
while True:
    ...
    if coordinateStore1.new:
        ...
        inp = torch.tensor([t0,t1, coordinateStore1.point[0]-x, coordinateStore1.point[1]-y]).float()
        inp -= data_mean
        ...
        t0 = api.getPos(0,module)
        t1 = api.getPos(1,module)
        api.setPos(t0+t[0], t1+t[1], module)
        # if not test_second:
        #     continue
