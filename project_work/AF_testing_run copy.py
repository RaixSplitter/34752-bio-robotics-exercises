...
class CoordinateStore:
    def __init__(self):
        ...
        self.predefined_idx = 0

        while len(self.predefined_points) < AF_THRESHHOLD:
            x = np.random.randint(235,485) # Markus
            y = np.random.randint(145,280)
            point = [x,y]
            dist = np.linalg.norm(np.array(point) - np.array(self.predefined_points[-1])) if self.predefined_points else float('inf')
            if dist > 50:
                self.predefined_points.append(point)
    ...
    def next_point(self):
        if not self.new:
            if self.predefined_idx >= len(self.predefined_points):
                self.predefined_idx = 0

            point = self.predefined_points[self.predefined_idx]
            self.point = point
            self.new = True
            self.predefined_idx += 1
            self.id += 1
...
for i in range(n_steps*n_trials):
    ...
    while active_run:
        ...
        if wait_for_confirm:
            ...
            if (datetime.now()-timer).seconds >= 0.05:
                move_step_counter += 1
                ...
                with open("run_log_af_toggle_fixed_points.csv", "a") as f:
                    timestamp = datetime.now().isoformat()
                    f.write(f"{p_id}, {coordinateStore1.id}, {coordinateStore1.point[0]}, {coordinateStore1.point[1]}, {adaptive_filter_toggle}, {dist[0]}, {dist[1]}, {timestamp}\n") # p_id, adaptive_filter_toggle, dist_x, dist_y, timestamp
